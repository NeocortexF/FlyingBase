...
public class StatisticMonthly extends JFrame {
    ...
    private void initComponents() {
        ...
        setTitle("Flying Base: Monthly Based Risks");
    ...
    private void bSelectActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_bSelectActionPerformed
        ...
        Color yellowMatrix = new Color(224, 214, 0);
        ...
        objHighRisks = new StatisticDBUpdater().getDataHighRiskTrend(startDate, finishDate);
        objMidRisks = new StatisticDBUpdater().getDataMidRiskTrend(startDate, finishDate);
        objLowRisks = new StatisticDBUpdater().getDataLowRiskTrend(startDate, finishDate);
        ...
        //разбиваем массив из БД - заполняем массив месяцев Low   === step 1 ===
        Object[] diagrammaDateListLow = null;
        ArrayList monthsOnDiagrammaLow = new ArrayList();
        for (int i = 1; i < objLowRisks.length; i = i + 2) {
            String dayMonthAndYear = objLowRisks[i].toString();
            ...
            monthsOnDiagrammaLow.add(monthAndYear);
        }
        diagrammaDateListLow = monthsOnDiagrammaLow.toArray();

        //разбиваем массив из БД - заполняем массив месяцев Mid
        Object[] diagrammaDateListMid = null;
        ArrayList monthsOnDiagrammaMid = new ArrayList();
        for (int i = 1; i < objMidRisks.length; i = i + 2) {
            String dayMonthAndYear = objMidRisks[i].toString();
            StringBuilder sb = new StringBuilder();
            sb.append(dayMonthAndYear.charAt(3));
            sb.append(dayMonthAndYear.charAt(4));
            sb.append(".");
            ...
            sb.append(dayMonthAndYear.charAt(9));
            String monthAndYear = sb.toString();
            monthsOnDiagrammaMid.add(monthAndYear);
        }
        diagrammaDateListMid = monthsOnDiagrammaMid.toArray();

        //разбиваем массив из БД - заполняем массив месяцев High
        Object[] diagrammaDateListHigh = null;
        ArrayList monthsOnDiagrammaHigh = new ArrayList();
        for (int i = 1; i < objHighRisks.length; i = i + 2) {
            String dayMonthAndYear = objHighRisks[i].toString();
            StringBuilder sb = new StringBuilder();
            sb.append(dayMonthAndYear.charAt(3));
            sb.append(dayMonthAndYear.charAt(4));
            sb.append(".");
            ...
            sb.append(dayMonthAndYear.charAt(9));
            String monthAndYear = sb.toString();
            monthsOnDiagrammaHigh.add(monthAndYear);
        }
        diagrammaDateListHigh = monthsOnDiagrammaHigh.toArray();

        //разбиваем массив из БД - заполняем массив кол-ва событий High  === step 2 ===
        Object[] diagrammaQuantityHigh = null;
        ArrayList quantityHigh = new ArrayList();
        for (int i = 0; i < objHighRisks.length; i = i + 2) {
            Object quant = objHighRisks[i];
            quantityHigh.add(quant);
        }
        diagrammaQuantityHigh = quantityHigh.toArray();

        //разбиваем массив из БД - заполняем массив кол-ва событий Mid
        Object[] diagrammaQuantityMid = null;
        ArrayList quantityMid = new ArrayList();
        for (int i = 0; i < objMidRisks.length; i = i + 2) {
            Object quant = objMidRisks[i];
            quantityMid.add(quant);
        }
        diagrammaQuantityMid = quantityMid.toArray();

        //разбиваем массив из БД - заполняем массив кол-ва событий Low
        Object[] diagrammaQuantityLow = null;
        ArrayList quantityLow = new ArrayList();
        for (int i = 0; i < objLowRisks.length; i = i + 2) {
            Object quant = objLowRisks[i];
            quantityLow.add(quant);
        }
        diagrammaQuantityLow = quantityLow.toArray();

        // заполняем хронологию массива дат от начала до конца    === step 3 ===
        ...
        //проверяем массив на пропушенные месяца, заполняем нулями пропуски Low
        Object[] diagrammaQuantityCheckedLow = null;
        ArrayList quantityCheckedLow = new ArrayList();
        int t = 0;
        for (int i = 0; i < checkedArrayForEmpties.length; i++) {
            if (checkedArrayForEmpties[i].equals(diagrammaDateListLow[t])) {
                quantityCheckedLow.add(diagrammaQuantityLow[t]);
                if (t < diagrammaDateListLow.length - 1) {
                    t++;
                }
            } else {
                quantityCheckedLow.add(0);
            }
        }
        diagrammaQuantityCheckedLow = quantityCheckedLow.toArray();

        //проверяем массив на пропушенные месяца, заполняем нулями пропуски Mid
        Object[] diagrammaQuantityCheckedMid = null;
        ArrayList quantityCheckedMid = new ArrayList();
        t = 0;
        for (int i = 0; i < checkedArrayForEmpties.length; i++) {
            if (checkedArrayForEmpties[i].equals(diagrammaDateListMid[t])) {
                quantityCheckedMid.add(diagrammaQuantityMid[t]);
                if (t < diagrammaDateListMid.length - 1) {
                    t++;
                }
            } else {
                quantityCheckedMid.add(0);
            }
        }
        diagrammaQuantityCheckedMid = quantityCheckedMid.toArray();

        //проверяем массив на пропушенные месяца, заполняем нулями пропуски High
        Object[] diagrammaQuantityCheckedHigh = null;
        ArrayList quantityCheckedHigh = new ArrayList();
        t = 0;
        for (int i = 0; i < checkedArrayForEmpties.length; i++) {
            if (checkedArrayForEmpties[i].equals(diagrammaDateListHigh[t])) {
                quantityCheckedHigh.add(diagrammaQuantityHigh[t]);
                if (t < diagrammaDateListHigh.length - 1) {
                    t++;
                }
            } else {
                quantityCheckedHigh.add(0);
            }
        }
        diagrammaQuantityCheckedHigh = quantityCheckedHigh.toArray();

        //заполняем дата сет для диаграммы
        for (int i = 0; i < checkedArrayForEmpties.length; i++) {
            barChartData.addValue(Integer.parseInt(diagrammaQuantityCheckedLow[i].toString()), "Низкий", checkedArrayForEmpties[i].toString());
        }
        for (int i = 0; i < checkedArrayForEmpties.length; i++) {
            barChartData.addValue(Integer.parseInt(diagrammaQuantityCheckedMid[i].toString()), "Умеренный", checkedArrayForEmpties[i].toString());
        }
        for (int i = 0; i < checkedArrayForEmpties.length; i++) {
            barChartData.addValue(Integer.parseInt(diagrammaQuantityCheckedHigh[i].toString()), "Высокий", checkedArrayForEmpties[i].toString());
        }

        //строим диаграмму
        JFreeChart barChart;
        if (rbHorizont.isSelected()) {
            barChart = ChartFactory.createBarChart3D("Помесячная статистика риска за период с " + startDateForGraph + " по " + finishDateForGraph, "Дата", "Количество событий", barChartData, PlotOrientation.VERTICAL, false, true, false);
        } else {
            barChart = ChartFactory.createBarChart3D("Помесячная статистика риска за период с " + startDateForGraph + " по " + finishDateForGraph, "Дата", "Количество событий", barChartData, PlotOrientation.HORIZONTAL, false, true, false);
        }

        CategoryPlot barChartCategory = barChart.getCategoryPlot();
        barChartCategory.setRangeGridlinePaint(Color.WHITE);


        //Альтернативная раскраска 
        final BarRenderer renderer = (BarRenderer) barChartCategory.getRenderer();
        ...
        renderer.setSeriesPaint(0, greenMatrix);
        renderer.setSeriesPaint(1, yellowMatrix);
        renderer.setSeriesPaint(2, orangeMatrix);
