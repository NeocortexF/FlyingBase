...
public class GopiPerson extends javax.swing.JFrame {
    ...
    private void retrieve() {
        DefaultTableModel userModel = new GopiDBUpdater().getData();
    ...
    private void initComponents() {
        ...
        jLabel1.setText("Перечень персонала ГОПИ");
        ...
        jLabel2.setText("*база данных автоматически сортирует персонал по ФИО в этом списке");
    ...
    private void jButton2ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButton2ActionPerformed
        ...
        GopiCreate gopiNewUser = new GopiCreate();
        Dimension screenSize = Toolkit.getDefaultToolkit().getScreenSize();
        int locX = (screenSize.width - 350) / 2;
        int locY = (screenSize.height - 260) / 2;
        gopiNewUser.setLocation(locX, locY);
        gopiNewUser.setVisible(true);
    ...
    private void bDeleteActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_bDeleteActionPerformed
        ...
        if (!tableUsers.getValueAt(tableUsers.getSelectedRow(), 0).equals("")) { //
            ...
            int n = JOptionPane.showOptionDialog(null, "Вы действительно собираетесь удалить сотрудника  " + userName + "?", "Предупреждение!", JOptionPane.YES_NO_OPTION,
                    JOptionPane.QUESTION_MESSAGE, null, options, options[1]);
            if (n == JOptionPane.YES_OPTION) {
                Boolean delete = new GopiDBUpdater().delete(userID);
            ...
            JOptionPane.showMessageDialog(null,
                    "Для удаления, пожалуйста, выберите сотрудника из списка",
    ...
    private void bEditActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_bEditActionPerformed
        ...
        if (!tableUsers.getValueAt(tableUsers.getSelectedRow(), 0).equals("")) {
            ...
            JOptionPane.showMessageDialog(null,
                    "Для детального просмотра, пожалуйста, выберите сотрудника из списка",
                    "Вы не выбрали сотрудника!",
                    JOptionPane.ERROR_MESSAGE);
        }
        GopiEdit gopiEditUser = new GopiEdit(userID);
        Dimension screenSize = Toolkit.getDefaultToolkit().getScreenSize();
        int locX = (screenSize.width - 350) / 2;
        int locY = (screenSize.height - 260) / 2;
        gopiEditUser.setLocation(locX, locY);
        gopiEditUser.setVisible(true);
